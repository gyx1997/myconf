...
class Paper extends \myConf\BaseModel
{
     ...
     */
    public function add(int $user_id, int $conference_id, array $authors, array $pdf_file_info, array $copyright_file_info, string $paper_type, string $suggested_session, string $title, string $abstract, int $paper_status = 0, string $custom_suggested_session = ''): int
    {
        //放在事务外面，读取不加写锁
     ...
     * @throws \myConf\Exceptions\DbTransactionException
     */
    public function update_paper_by_id(int $paper_id, array $authors = null, array $pdf_file_info = null, array $copyright_file_info = null, string $paper_type = null, string $suggested_session = null, string $title = null, string $abstract = null, string $custom_suggested_session = null): void
    {
        //var_dump($suggested_session, $paper_type, $custom_suggested_session);
        //
    ...
    {
        $paper = $this->Tables->Papers->get(strval($paper_id));
        $authors = $this->Tables->PaperAuthors->fetch_all(['paper_id' => $paper_id]);
        $paper_session = $this->Tables->PaperSessions->get($paper['suggested_session']);
        DbHelper::begin_trans();
        ...
        //删除paper信息
        $this->Tables->Papers->delete(strval($paper_id));
        //删除对应的附件信息
