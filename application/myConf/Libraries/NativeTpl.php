...
define('TEMPLATE_COMPILED_PATH', APPPATH . 'cache' . DIRECTORY_SEPARATOR . 'template' . DIRECTORY_SEPARATOR);

class NativeTpl
{
    /**
     * @param string $source 模板源文件
     * @param bool $force_compile 是否强制重新编译
     * @return string
     * @throws TemplateParseException
     * @throws \myConf\Exceptions\TemplateNotFoundException
     */
    public static function load(string $source, bool $force_compile = false): string
    {
        ...
        if (
            ...
            self::_compile($source, $target_tpl_file);
        }
     ...
     * @throws \myConf\Exceptions\TemplateNotFoundException
     */
    public static function _compile(string $source, string $target): void
    {
        $source_content = self::_parse_template($source, true);
        $source_content = '<?php defined(\'MY_CONF\') OR exit(\'No direct script access allowed\'); ?>' . PHP_EOL . $source_content;
        ENVIRONMENT === 'production' && $source_content = self::_compress($source_content);
     ...
     * @throws \myConf\Exceptions\TemplateNotFoundException
     */
    private static function _parse_template(string $include_item, bool $full_path = false): string
    {
        $file = !$full_path ? TEMPLATE_BASE_PATH . $include_item : $include_item;
        $file = str_replace(array('/', '\\'), DIRECTORY_SEPARATOR, $file);
        $file = $file . '.html';
        if (!file_exists($file)) {
            throw new \myConf\Exceptions\TemplateNotFoundException('TPL_NOT_FOUND', 'Template "' . $include_item . '"  not found.');
        }
        $content = file_get_contents($file);
        $t = self::_parse_tags($content);
     ...
     * @throws TemplateParseException
     */
    private static function _parse_tags(string &$content): array
    {
        //栈，用来追踪end标记
        $st = new \SplStack();
        //约定语义动作
        ...
        $tag_actions = array(
            'include' => function (string &$param_str, \SplStack &$st): string {
                ...
                if (strpos($param_str, ' ') === false) {
                    return self::_parse_template($param_str);
                }
    ...
    private static function _compress(string $template_str): string
    {
