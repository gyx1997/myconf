...
class PaperSessions extends \myConf\BaseEntityTable
{
    ...
    {
        return DbHelper::make_table('paper_sessions');
    }
    ...
    public function get_conference_sessions(int $conference_id, bool $force_from_db = false) : array {
        ...
        if ($force_from_db === true) {
            $data = $this->fetch_all(['session_conference_id' => $conference_id], 'session_display_order', 'ASC');
            $data_ids = [];
    ...
    public function get_conference_sessions_max_display_order(int $conference_id) : int {
        $display_order_max = DbHelper::fetch_first_raw('SELECT MAX(session_display_order) FROM ' . $this->table() . ' WHERE session_conference_id = ' . $conference_id)['MAX(session_display_order)'];
